                            ...
                            <form id="salaryForm"
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-4 mb-3">
                                        <label class="form-label fw-bold">Absent Deduction Method</label>
                                        <div class="d-flex gap-3 pt-2">
                                            <div class="form-check">
                                                <input class="form-check-input" type="radio" name="absent_deduction_type" id="deduction_manual" value="manual" 
                                                    {{ ($salaryStructure->absent_deduction_type ?? 'manual') == 'manual' ? 'checked' : '' }}
                                                    {{ $readOnly ?? false ? 'disabled' : '' }}>
                                                <label class="form-check-label" for="deduction_manual">Manual</label>
                                            </div>
                                            <div class="form-check">
                                                <input class="form-check-input" type="radio" name="absent_deduction_type" id="deduction_automatic" value="automatic"
                                                    {{ ($salaryStructure->absent_deduction_type ?? 'manual') == 'automatic' ? 'checked' : '' }}
                                                    {{ $readOnly ?? false ? 'disabled' : '' }}>
                                                <label class="form-check-label" for="deduction_automatic">Automatic</label>
                                            </div>
                                        </div>
                                    </div>

                                    <!-- Leave Salary Per Day / Absent Deduction Amount -->
                                    <div class="col-md-4 mb-3">
                                        <label class="form-label fw-bold">Absent Deduction (Per Day)</label>
                                        <input type="number" step="0.01" name="leave_salary_per_day" id="leave_salary_per_day"
                                            class="form-control" value="{{ $salaryStructure->leave_salary_per_day ?? '' }}"
                                            placeholder="Amount to deduct per absent day" 
                                            {{ ($readOnly ?? false) || (($salaryStructure->absent_deduction_type ?? 'manual') == 'automatic') ? 'disabled' : '' }}>
                                        <small class="text-muted" id="absent_deduction_note">
                                            {{ ($salaryStructure->absent_deduction_type ?? 'manual') == 'automatic' ? 'System will automatically calculate based on working hours.' : 'Enter fixed amount to deduct for each absent day.' }}
                                        </small>
                                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            function updateStructureUI() {
                ...
                // Absent Deduction Logic
                $('input[name="absent_deduction_type"]').on('change', function() {
                    const type = $(this).val();
                    if (type === 'automatic') {
                        $('#leave_salary_per_day').prop('disabled', true);
                        $('#absent_deduction_note').text('System will automatically calculate based on working hours.');
                    } else {
                        $('#leave_salary_per_day').prop('disabled', false);
                        $('#absent_deduction_note').text('Enter fixed amount to deduct for each absent day.');
                    }
                    calculateSummary();
                });
