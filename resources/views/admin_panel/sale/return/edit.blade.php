...
<div class="container-fluid">
    <div class="card shadow-sm border-0 mt-3">
        <div class="card-header ">
            <h5 class="mb-0 text-dark">EDIT SALES RETURN</h5>
            <a href="{{ url()->previous() }}" class="btn btn-danger btn-sm">Back</a>
        </div>

        <form action="{{ route('sale.returns.update', $salesReturn->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="card-body">
                    ...
                    <table class="table table-bordered table-sm align-middle text-center" id="returnItemsTable">
                        ...
                        <tbody>
                            ...
                            <tr data-product-id="{{ $item['product_id'] }}">
                                <td class="text-start">
                                    {{ $item['item_name'] }}
                                    <input type="hidden" name="product[]" value="{{ $item['item_name'] }}">
                                    <input type="hidden" name="product_id[]" value="{{ $item['product_id'] }}">
                                    <input type="hidden" name="item_code[]" value="{{ $item['item_code'] }}">
                                </td>
                                <td>{{ $item['item_code'] }}</td>
                                <td>
                                    <textarea name="color[]" class="form-control form-control-sm note-textarea" rows="2" readonly>{{ $item['note'] }}</textarea>
                                </td>
                                <td>
                                    {{ $item['brand'] }}
                                ...
                                <td>
                                    <input type="number" step="0.01" name="price[]" class="form-control form-control-sm price-input" value="{{ $item['price'] }}">
                                </td>
                                <td>
                                    <input type="number" step="0.01" name="item_disc[]" class="form-control form-control-sm disc-input" value="{{ $item['discount'] }}">
                                </td>
                                <td>
                                    <input type="number" step="0.01" name="qty[]" class="form-control form-control-sm qty-input" value="{{ $item['return_qty'] }}" min="0.01" max="{{ $item['available_qty'] + $item['return_qty'] }}">
                                    <div class="small-muted">Max: <span class="max-qty">{{ $item['available_qty'] + $item['return_qty'] }}</span></div>
                                </td>
                                <td>
                                    <input type="text" name="total[]" class="form-control form-control-sm row-total" value="{{ number_format(($item['price'] * $item['return_qty']) - $item['discount'], 2, '.', '') }}" readonly>
                                </td>
                                <td>
                ...
                <table class="table table-bordered table-sm mt-3 text-center">
                    ...
                    <tr>
                        ...
                        <td><input type="number" id="extraDiscount" name="total_extra_cost" class="form-control form-control-sm text-center" value="{{ $salesReturn->total_extradiscount ?? 0 }}"></td>
                        <td><input type="text" id="netAmount" name="total_net" class="form-control form-control-sm text-center" readonly></td>
                        <td><input type="number" id="cash" name="cash" class="form-control form-control-sm text-center" value="{{ $salesReturn->cash ?? 0 }}"></td>
                        <td><input type="number" id="card" name="card" class="form-control form-control-sm text-center" value="{{ $salesReturn->card ?? 0 }}"></td>
                ...
                <div class="d-flex justify-content-between align-items-center mt-3">
                    ...
                    <div>
                        <button type="submit" class="btn btn-success">Update Sale Return</button>
