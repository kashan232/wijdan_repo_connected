...
<div class="main-content">
    <div class="container-fluid">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h3 class="fw-bold">Edit Expense Voucher</h3>
        </div>
                ...
                <form action="{{ route('expense.voucher.update', $voucher->id) }}" method="POST">
                    ...
                    <div class="row mb-4">
                        <div class="col-md-2">
                            ...
                            <input type="text" class="form-control form-control-sm"
                                name="evid" value="{{ $voucher->evid }}" readonly>
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="vendor_type" class="form-select form-select-sm">
                                ...
                                <option value="{{ $head->id }}" {{ $voucher->type == $head->id ? 'selected' : '' }}>{{ $head->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="vendor_id" class="form-select form-select-sm">
                                ...
                                <option value="{{ $acc->id }}" {{ $voucher->party_id == $acc->id ? 'selected' : '' }}>{{ $acc->title }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-2">
                            ...
                            <input type="date" class="form-control form-control-sm"
                                name="date" value="{{ \Carbon\Carbon::parse($voucher->date)->format('Y-m-d') }}">
                        </div>
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-sm align-middle" id="voucherTable">
                            ...
                            <tbody>
                                @php
                                    $remarks = is_array($voucher->remarks) ? $voucher->remarks : (json_decode($voucher->remarks, true) ?? ['']);
                                    $amounts = is_array($voucher->amount) ? $voucher->amount : (json_decode($voucher->amount, true) ?? [0]);
                                @endphp
                                ...
                                <tr>
                                    ...
                                    <th>
                                        <input type="text" id="totalAmount"
                                            ...
                                            readonly value="{{ number_format($voucher->total_amount, 2, '.', '') }}">
                                    </th>
                                    <th></th>
                                </tr>
                            </tfoot>
                    ...
                    <div class="mt-4">
                        ...
                        <a href="{{ route('all-expense-vochers') }}" class="btn btn-outline-secondary px-4">Exit</a>
                    </div>
